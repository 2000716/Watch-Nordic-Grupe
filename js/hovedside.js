...
function oppdaterBanner() {
  try {
    ...
    if (addBtn && film.tittel) {
      ...
      const erIListe = liste.includes(key);

      addBtn.innerHTML = erIListe ? '<i class="fas fa-check"></i>' : '<i class="fas fa-plus"></i>';
      addBtn.setAttribute("aria-label", erIListe ? "Fjern fra Min Liste" : "Legg til i Min Liste");

      addBtn.onclick = () => {
        let currentListe = sikkerJSONParse(localStorage.getItem("minListe"), []);
        const finnes = currentListe.includes(key);
        if (finnes) {
        ...
        const naHar = currentListe.includes(key);
        addBtn.innerHTML = naHar ? '<i class="fas fa-check"></i>' : '<i class="fas fa-plus"></i>';
        addBtn.setAttribute("aria-label", naHar ? "Fjern fra Min Liste" : "Legg til i Min Liste");
      };
    }
...
window.addEventListener("resize", debounce(oppdaterBanner, 200));
...
window.addEventListener("resize", debounce(oppdaterAlleGalleripiler, 200));
...
function createGalleryItem(itemKey, item) {
  ...
  link.innerHTML = `
    ${
      bildeKilde
        ? `<img src="${escapeHTML(bildeKilde)}" alt="${tittelTekst}" loading="lazy">`
        : `<div style="width:100%; height:100%; background:#1a2629; display:flex; align-items:center; justify-content:center; padding:10px; text-align:center; font-size:12px; color:#fff;">${tittelTekst}</div>`
    }
...
async function hentDataFraFirestore() {
  ...
  try {
    ...
    const nyeFilmer = {};
    filmerSnap.forEach((docSnap) => {
      nyeFilmer[docSnap.id] = docSnap.data();
    });

    const nyeSerier = {};
    serierSnap.forEach((docSnap) => {
      nyeSerier[docSnap.id] = docSnap.data();
    });

    const nyeFilmerStr = JSON.stringify(nyeFilmer);
    const nyeSerierStr = JSON.stringify(nyeSerier);

    if (nyeFilmerStr !== lagredeFilmer || nyeSerierStr !== lagredeSerier) {
      localStorage.setItem(cacheKeyFilmer, nyeFilmerStr);
      localStorage.setItem(cacheKeySerier, nyeSerierStr);
...
function byggSiden() {
  try {
    ...
    console.error("Feil under bygging av siden:", err);
  }
}

// ==================== INITIALISERING ==================== //
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", hentDataFraFirestore);
} else {
