<script type="module">
  ...
  async function init(eksplisittId = null) {
    data = null; // Tilbakestiller forrige visning
  ...
  async function byggAnbefalingerEllerEpisoder() {
    ...
    if (type === "serie" && data.sesonger) {
      ...
      if (!gallery) return;
      gallery.innerHTML = ""; // Tømmer for ny SPA-visning
  ...
  // Eksporteres til window så ruting-skriptet ditt enkelt kan kalle denne visningen
  window.lastFilminfoMedId = function(medieId) {
    init(medieId);
  };

  // Reagerer på hash-endringer dersom din SPA bruker window.location.hash
