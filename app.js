...
// ==================== 3. SIDEBYTTE & VISNINGSSTYRING (SPA) ==================== //
...
function oppdaterBanner() {
  try {
    ...
    if (seNa) {
      seNa.onclick = () => { 
        if (film.videoUrl) {
          apneAvspiller(film.videoUrl, film.tittel);
        } else {
          window.location.href = `film.html?navn=${encodeURIComponent(film.key)}`;
        }
      };
    }
    ...
    bannerTimer = setTimeout(() => {
      ...
      video.currentTime = 0;
      video.play().catch((err) => console.log("Auto-play hindret:", err));
      video.onended = () => {
        video.style.display = "none";
        bilde.style.display = "block";
      };
...
function renderContinueWatching() {
  try {
    ...
    sorterteFilmIder.forEach((filmId) => {
      ...
      if (skjulteTitler.includes(filmId)) return;
      ...
      if (erInnloggetSomBarn && (!erEgnetForBarn(metadataMatch) || !sjekkAldersgrense(metadataMatch))) return;

      fantInnhold = true;

      const hrefUrl = spillType === "serie"
        ? `film-mal.html?navn=${encodeURIComponent(urlParamNavn)}&sesong=${encodeURIComponent(sesongNr)}&episode=${encodeURIComponent(episodeNr)}`
        : `film.html?navn=${encodeURIComponent(urlParamNavn)}`;
      ...
      if (spillType === "serie" && metadataMatch.sesonger && metadataMatch.sesonger[sesongNr]) {
        ...
        if (epInfo && epInfo.bilde) bildeKilde = epInfo.bilde;
      }
      ...
      const episodeTekst = spillType === "serie" && sesongNr && episodeNr ? ` • S${sesongNr}E${episodeNr}` : "";
      const tittelClean = escapeHTML(metadataMatch.tittel || "");

      link.innerHTML = `
        <div class="continue-media-wrap">
          ${bildeKilde ? `<img src="${escapeHTML(bildeKilde)}" alt="${tittelClean}" loading="lazy">` : `<div style="width:100%; height:100%; background:#1a2629; display:flex; align-items:center; justify-content:center; padding:10px; text-align:center; font-size:12px; color:#fff;">${tittelClean}</div>`}
      ...
      deleteBtn.addEventListener("click", async (e) => {
        ...
        if (!Array.isArray(profil.skjultFortsett)) profil.skjultFortsett = [];
        if (!profil.skjultFortsett.includes(filmId)) profil.skjultFortsett.push(filmId);
        ...
        if (registrertBruker) {
          try {
            ...
            await updateDoc(userDocRef, { profiler: heleProfilArrayet });
          } catch (err) {
            console.error("Feil ved skjuling i Firebase:", err);
          }
        }
        ...
        setTimeout(() => {
          link.remove();
          if (gallery.children.length === 0) section.style.display = "none";
          else oppdaterAlleGalleripiler();
        }, 300);
      });
...
function initialiserAvspillerKontroller() {
  ...
  if (bannerSeNa && !bannerSeNa.onclick) {
...
// ==================== GLOBAL EKSPONERING FOR INLINE HTML ==================== //
...
window.oppdaterAlleGalleripiler = oppdaterAlleGalleripiler;
